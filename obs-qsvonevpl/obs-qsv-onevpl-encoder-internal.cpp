...
// Rebuild SPS, inserting the 8 scaling lists (4x4 x6 + 8x8 x2) when preset is
// not DEFAULT. Returns new NAL size (with start code + EP), 0 on failure.
static mfxU16 H264RebuildSps(const H264SpsFields &s, int preset,
  ...
  if (s.is_high) {
    ...
    if (preset == QM_DEFAULT) {
      w.u(0, 1);  // seq_scaling_matrix_present_flag = 0
    } else {
      w.u(1, 1);
      H264MatrixSet m = GetH264Matrices(preset);
      uint8_t flat16[16], flat64[64];
      const uint8_t *i4 = m.i4, *p4 = m.p4, *i8 = m.i8, *p8 = m.p8;
      if (preset == QM_FLAT16) {
        for (int i = 0; i < 16; i++) flat16[i] = 16;
        for (int i = 0; i < 64; i++) flat64[i] = 16;
        i4 = p4 = flat16;
        i8 = p8 = flat64;
      }
      // groups: 0..5 = 4x4 (IntraY, IntraCb, IntraCr, InterY, InterCb,
      // InterCr), 6..7 = 8x8 (IntraY8, InterY8). Chroma copies luma.
      const uint8_t *groups4[6] = {i4, i4, i4, p4, p4, p4};
      for (int i = 0; i < 6; i++) {
        w.u(1, 1);
        H264WriteScalingList(w, groups4[i], 16);
      }
      const uint8_t *groups8[2] = {i8, p8};
      for (int i = 0; i < 2; i++) {
        w.u(1, 1);
        H264WriteScalingList(w, groups8[i], 64);
      }
    }
  }
...
static mfxU16 H264RebuildPps(const H264PpsFields &p, int preset,
  ...
  bool write8x8 = p.transform8x8 && preset != QM_DEFAULT;
  if (p.has_extra) {
      ...
      if (GvpSts >= MFX_ERR_NONE && SPSPPSParams->SPSBufSize > 0 &&
        ...
        if (wantMatrix && ssc > 0) {
          ...
          if (H264ParseSps(spsRbsp.data() + ssc + 1,
              ...
              sf.is_high) {
            const mfxU16 n = H264RebuildSps(sf, qmPreset, spsHdr, ssc,
                                            NewSps.data(),
        ...
        const int effPreset = (wantMatrix && spsOk) ? qmPreset : QM_DEFAULT;
        if (psc > 0) {
          const uint8_t ppsHdr = ppsRbsp[psc];
          H264PpsFields pf;
          if (H264ParsePps(ppsRbsp.data() + psc + 1,
                           ppsRbsp.size() - psc - 1, pf)) {
            const mfxU16 n = H264RebuildPps(pf, effPreset, chromaOffset, ppsHdr,
                                            psc, NewPps.data(),
        ...
        const bool needInject = ppsOk && (effPreset != QM_DEFAULT || wantChroma);
        if (needInject) {
...
// Non-texture encoder path selection:
    ...
    FrameStatsEnabled = InputParams->FrameStatistics;
...
mfxStatus QSVEncoder::SetEncoderParams(struct encoder_params *InputParams,
  ...
  case MFX_RATECONTROL_AVBR:
    ...
    QSVEncodeParams.mfx.BufferSizeInKB =
        static_cast<mfxU16>(QSVEncodeParams.mfx.TargetKbps / 4);
    ApplyBufferSettings();
    // AVBR: Accuracy shares union with InitialDelayInKB, Convergence with MaxKbps.
    // For AVBR, those are unused (non-HRD), so safe.
    QSVEncodeParams.mfx.Accuracy = InputParams->Accuracy;
    QSVEncodeParams.mfx.Convergence = InputParams->Convergence;
    info("\tAVBR Accuracy: %d (tenth of %%, UI %.1f%%), Convergence: %d (x100 frames)",
         InputParams->Accuracy,
         static_cast<double>(InputParams->Accuracy) / 10.0,
         InputParams->Convergence);
    break;
...

  {
    ...
    // ExtBRC is intentionally kept OFF:
    // - On this GPU it causes MFX_ERR_DEVICE_FAILED (-16) at first frame
    // - LA mode (VBR+Lookahead) also breaks — 40x target bitrate
    // - Driver forces it OFF for non-CBR/VBR modes anyway
    //   (mfx_h264_enc_common_hw.cpp:3810-3814)
    if (!IsUHD600HEVC) {
    ...
    //   AVC:   CBR/VBR/ICQ. CBR = EncTools LAGS hardware, VBR/ICQ = promoted to LA.
...

  {
    ...
    // Set NumRefActive to match NumRefFrame; driver will clamp to HW max
    {
      mfxU16 refActive = QSVEncodeParams.mfx.NumRefFrame;
      std::fill(CO3Params->NumRefActiveP, CO3Params->NumRefActiveP + 8, refActive);
      std::fill(CO3Params->NumRefActiveBL0, CO3Params->NumRefActiveBL0 + 8, refActive);
      std::fill(CO3Params->NumRefActiveBL1, CO3Params->NumRefActiveBL1 + 8, refActive);
    }
...
#endif
    // ExtBRC is always OFF — causes MFX_ERR_DEVICE_FAILED on this GPU.
    // AdaptiveMBQP and BRC work with the driver's built-in BRC, not ExtBRC.
...
void QSVEncoder::LogQPStats() {
  blog(LOG_INFO,
       "[QSV VPL] QP statistics (total %llu frames):",
  ...
  auto logType = [](const char *label, const QPFrameTypeStats &s) {
    ...
    blog(LOG_INFO,
         "  %s: count=%llu  min=%u  max=%u  avg=%.2f  median=%u",
         label,
...
void QSVEncoder::LogFrameStats() {
  blog(LOG_INFO,
       "[QSV VPL] Frame statistics (total %llu frames, %llu bytes):",
  ...
  logType("I-frames", Stats.i);
  logType("P-frames", Stats.p);
  logType("B-frames", Stats.b);

  if (Stats.totalPanicFrames > 0) {
    blog(LOG_INFO, "  BRC Panic frames: %llu",
         static_cast<unsigned long long>(Stats.totalPanicFrames));
  }
}
